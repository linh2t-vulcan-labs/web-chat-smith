...
import { PRIVACY_POLICY_URL } from "@/utils/constants/url";

export const revalidate = 3600; // must be a static literal in Next 16 (was env.SANITY_REVALIDATE_TIME, default 3600)

const POLICY_SLUG = "privacy-policy" as const;

const getPrivacyPolicy = cache((lang: string) =>
...
export async function generateMetadata(
  ...
  return generatePolicyMetadata(
    ...
    PRIVACY_POLICY_URL,
    {
      ...
      title: "Chat Smith - Privacy Policy",
    },
    { hrefLangLocales }
  );
}

export default async function PrivacyPolicyPage(props: TPolicyPageProps) {
  const params = await props.params;
  const lang = normalizeAppLocale(params.locale);
  const { data: policy, error } = await getPrivacyPolicy(lang);
  ...
  const title = policy.seo?.title?.trim() || "Chat Smith - Privacy Policy";
