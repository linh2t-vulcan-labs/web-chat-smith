...
**Verify trực tiếp trên SDK đã cài** (`@firebase/remote-config@0.9.1`, bundle theo `firebase@12.16/12.17`, không suy luận từ docs chung chung): type declaration gốc (`remote-config-public.d.ts`, `src/index.d.ts`) ghi thẳng "This SDK does not work in a Node.js environment". Bên trong, code có fallback `InMemoryStorage` khi thiếu IndexedDB (không throw ngay lúc `getRemoteConfig()`), nhưng Firebase không support/test path đó — đừng dựa vào nó. **`FirebaseServerApp` (tính năng SSR chính thức của Firebase, mở rộng Auth/Firestore/App Check sang server) không mở rộng tới Remote Config** — cảnh báo trên nằm ở tầng package `@firebase/remote-config` và không phụ thuộc việc `FirebaseApp` được khởi tạo kiểu gì (thường hay `FirebaseServerApp`), nên dùng `initializeServerApp` + `getRemoteConfig(serverApp)` theo đúng pattern Firebase gợi ý cho Auth cũng không đổi được kết luận này.
...
**Quyết định: client-only, vĩnh viễn — không phải "mặc định tạm thời"**. Đã đánh giá và **từ chối** cả 2 hướng server-side dưới đây, lý do ghi lại tại đây để không ai phải đánh giá lại từ đầu:
...
3. **`useFlag` đã resolve đồng bộ** (`packages/flags/src/react/create-flags-react.tsx`) — trả về `defaultValue` của schema ngay lần render đầu, không đợi Remote Config fetch xong. `<Suspense>` ở `Feature`/`ExperimentSwitch` chỉ tồn tại để code-split các variant (`next/dynamic`/`React.lazy`), **không phải** để chờ giá trị flag. Nói cách khác: flicker do "chờ RC" không tồn tại ở tầng UI — flag luôn có giá trị ngay từ frame đầu (default hoặc giá trị cache cũ), Remote Config chỉ âm thầm refine giá trị đó sau khi `fetchAndActivate()` xong. Đây là lý do chính khiến việc cố đưa Remote Config lên RSC không mang lại lợi ích UX thật.

**Đã cân nhắc và từ chối 2 hướng server-side sau** (SEO-critical content phụ thuộc flag, hoặc tránh layout shift — tình huống duy nhất còn có thể cần tới):

- **`firebase-admin`'s server-side Remote Config template API** (`getServerTemplate()`) — về mặt kỹ thuật đây là cách hợp lệ duy nhất để đọc Remote Config trên Node (khác `firebase/remote-config` client SDK, `firebase-admin` không bị giới hạn IndexedDB). Nhưng chi phí vận hành (1 service account credential nhạy cảm mới, cần Vault/devops cấp phát) lớn hơn lợi ích thực tế — nhất là khi mục 3 ở trên đã loại bỏ phần lớn lý do "tránh flicker". Nếu sau này thực sự phát sinh nhu cầu SEO/layout-shift cụ thể, đây vẫn là hướng đúng để quay lại — nhưng phải đi kèm 1 lý do cụ thể, không phải "để tối ưu chung chung".
- **Redis/KV cache** cho giá trị Remote Config đã materialize — **không cần thiết ngay cả khi theo đuổi hướng Admin SDK ở trên**: dữ liệu Remote Config là global, ít thay đổi, không cần đồng bộ tuyệt đối giữa các pod GKE — cache per-process qua `"use cache"`/`unstable_cache` (Next's Data Cache có sẵn) là đủ, mỗi pod tự poll Firebase độc lập theo `revalidate`. Redis chỉ đáng request nếu có nhu cầu state dùng chung thật sự khác (rate-limit tập trung, pub/sub realtime, distributed lock) — không được lấy Remote Config làm lý do request Redis.
- **Cookie-based cache** cũng bị loại: Remote Config là dữ liệu global giống nhau cho mọi user, lưu qua cookie sẽ nhân bản dữ liệu theo từng user thay vì dùng chung 1 bản, không giải quyết được cold-start (request đầu tiên của mỗi user vẫn phải fetch), và có rủi ro va giới hạn ~4KB/cookie nếu schema flags phình to.
