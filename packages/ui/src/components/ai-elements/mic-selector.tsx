...
export const useAudioDevices = () => {
  ...
  useEffect(() => {
    // oxlint-disable-next-line react/react-compiler -- initial device-list fetch on mount, an external browser API not derivable from React state
    latest.current.loadDevicesWithoutPermission();
  }, []);

  useEffect(() => {
    const handleDeviceChange = () => {
      if (latest.current.hasPermission) {
        latest.current.loadDevicesWithPermission();
      } else {
        latest.current.loadDevicesWithoutPermission();
      }
    };
